...
final class TokenRegistry
{
    ...
    private final Map<ApiConnectionInfo, ReentrantLock> _urlSemaphoreMap = new ConcurrentHashMap<>();

    private TokenRegistry()
    {
    }

    public static TokenRegistry getRegistry()
    {
        return SingletonHelper.INSTANCE;
    }

    public TokenContainer getToken(ApiConnectionInfo connectionInfo, String username, char[] password) throws HttpAccessException
    {
        ...
        {
            //one lock per url
            ReentrantLock lock = _urlSemaphoreMap.computeIfAbsent(connectionInfo, k -> new ReentrantLock());
            try
            {
